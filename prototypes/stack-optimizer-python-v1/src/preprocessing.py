...
"""
def apply_moves_and_calculate_ERMs(initial_bay, moves):
    def perform_move(bay, move):
        from_stack, to_stack = move
        if bay[from_stack]:
            container = bay[from_stack].pop()
            bay[to_stack].append(container)
        return bay
    
    current_bay = initial_bay.copy()
    ERMs = {(0, None, None): calculate_expected_relocation_moves(current_bay)}
    
    for idx, move in enumerate(moves, start=1):
        current_bay = perform_move(current_bay, move)
        ERMs[(idx, move[0], move[1])] = calculate_expected_relocation_moves(current_bay)
...
#ERMs = apply_moves_and_calculate_ERMs(bay, P)


def apply_moves_and_calculate_ERMs_for_multiple_bays(all_bays, all_moves):
    def perform_move(bay, move):
        from_stack, to_stack = move
        if bay[from_stack]:
            container = bay[from_stack].pop()
    ...
    for bay_key, initial_bay in all_bays.items():
        current_bay = initial_bay.copy()
        ERMs = {(0, None, None): calculate_expected_relocation_moves(current_bay)}
        
        for idx, move in enumerate(all_moves.get(bay_key, []), start=1):
            current_bay = perform_move(current_bay, move)
            ERMs[(idx, move[0], move[1])] = calculate_expected_relocation_moves(current_bay)
        
        results[bay_key] = ERMs

    return results


#results = apply_moves_and_calculate_ERMs_for_multiple_bays(all_bays, all_moves)
