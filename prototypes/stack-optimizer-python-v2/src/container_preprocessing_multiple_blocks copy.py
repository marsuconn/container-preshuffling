...
if __name__ == "__main__":
    ...
    data_containers = pd.read_csv('./preprocessing/input/container_data_for_optimization.csv')
    #data_blocks = pd.read_excel('./preprocessing/input/input_samples_datetime_mutliple_blocks.xlsx')  
    block_config  = {'R':{'max_stacks': 5, 'max_tiers': 4},'JAX INTL':{'max_stacks': 5, 'max_tiers': 4}}

    df_containers = process_appointment_data(data_containers)
    
    input_bays=create_input_data(df_containers, block_config)
